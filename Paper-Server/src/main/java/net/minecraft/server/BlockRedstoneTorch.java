...
public class BlockRedstoneTorch extends BlockTorch {
    ...
    private boolean a(World world, BlockPosition blockposition, boolean flag) {
        if (!BlockRedstoneTorch.b.containsKey(world)) {
            BlockRedstoneTorch.b.put(world, Lists.<RedstoneUpdateInfo>newArrayList()); // CraftBukkit - fix decompile error
        }
    ...
    public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
        ...
        if (list != null) {
            int index = 0;
            while (index < list.size() && world.getTime() - ((RedstoneUpdateInfo) list.get(index)).getTime() > 60L) {
                index++;
            }
            if (index > 0) {
